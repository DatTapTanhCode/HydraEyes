...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("keydown", function (e) {
    const calibrationModalActive = document
      .getElementById("analogCalibrationModal")
      ?.classList.contains("active");
    const valueModalActive = document
      .getElementById("valueInputModal")
      ?.classList.contains("active");
  ...
  const configCameraAddressInput = document.getElementById(
    "configCameraAddress"
  );
  ...
  if (demoModeCheckbox && addressGroup) {
    demoModeCheckbox.addEventListener("change", function () {
      ...
      if (this.checked) {
        ...
        cameraAddressInput.placeholder =
          "Enter camera IP or URL (e.g., 192.168.1.100 or http://camera-url)";
      }
    });
  }
  ...
  if (configDemoModeCheckbox && configAddressGroup) {
    configDemoModeCheckbox.addEventListener("change", function () {
      if (this.checked) {
        ...
        configCameraAddressInput.placeholder =
          "Enter camera IP or URL (e.g., 192.168.1.100 or http://camera-url)";
      }
    });
  }
  ...
  const configDetectorTypeSelect =
    document.getElementById("configDetectorType");
  ...
  document
    ...
    .addEventListener("submit", async function (e) {
      ...
      if (!type || !detectorType || !location || isNaN(threshold)) {
        alert(
          "Please fill in all required fields (Detector Type, Gauge Type, Location, and Threshold)"
        );
        return;
      }
      ...
      try {
        ...
        if (demoMode) {
          ...
          console.log(
            `Demo camera ${newCamera.id} initialized with reading: ${
              gaugeReadings[newCamera.id]
            }`
          );
        } else {
          ...
          console.log(
            `Real camera ${newCamera.id} initialized with reading: 0 (awaiting detector)`
          );
        }
        ...
        const modeText = demoMode
          ? "(demo mode)"
          : `(${detectorType} detector)`;
        addActivityLog(
          `New ${detectorType} camera ${newCamera.id} added at ${location} (${type}) ${modeText}`,
          "success"
        );
  ...
  document
    ...
    .addEventListener("submit", async function (e) {
      ...
      const address = document
        .getElementById("configCameraAddress")
        .value.trim();
      ...
      const location = document
        .getElementById("configCameraLocation")
        .value.trim();
      const threshold = parseFloat(
        document.getElementById("configThresholdValue").value
      );
      ...
      try {
        ...
        if (oldCamera.detector_type !== detectorType)
          changes.push("detector type");
        ...
        if (changes.length > 0) {
          ...
          addActivityLog(
            `Camera ${cameraId} updated: ${changes.join(
              ", "
            )} changed (now ${detectorType} in ${modeText})`,
            "activity"
          );
        }
  ...
  document.getElementById("teamForm").addEventListener("submit", function (e) {
    ...
    document
      .querySelectorAll('#gaugeCheckboxes input[type="checkbox"]:checked')
      .forEach((checkbox) => {
...
async function fetchCameraReadings() {
  try {
    ...
    Object.keys(readings).forEach((cameraId) => {
      ...
      console.log(
        `Backend reading for ${cameraId}: ${readingData.value} (was: ${previousValue})`
      );
      ...
      if (readingData.value !== null && readingData.value !== undefined) {
        ...
        chartData[cameraId] = chartData[cameraId].filter(
  ...
  try {
    ...
    cameras.forEach((camera) => {
      if (camera.demo_mode) {
        ...
        console.log(
          `Existing demo camera ${camera.id} initialized with reading: ${
            gaugeReadings[camera.id]
          }`
        );
      } else {
        ...
        console.log(
          `Existing real camera ${camera.id} initialized with reading: 0`
        );
      }
...
function generateRandomReading(camera) {
  ...
  if (
    state.isOverThreshold &&
    state.overThresholdStartTime &&
    ...
    const shouldBreachThreshold =
      Math.random() < 0.03 && !state.isOverThreshold;
...
function generateFeedContent(camera) {
  if (!camera.address && !camera.demo_mode) {
    return `
            <div class="gauge-placeholder">
                <div style="font-size: 24px; margin-bottom: 8px;">${getTypeIcon(
                  camera.type
                )}</div>
  ...
  let content = `
        <div class="live-reading" id="reading-${camera.id}">${
    currentReading || 0
  }</div>
        <div class="reading-unit">${config.unit}</div>
        <div class="reading-status status-${status}-reading" id="status-${
    camera.id
  }">
            ${status.toUpperCase()}
        </div>
  ...
  if (cameras.length === 0) {
    ...
    const camerasHTML = cameras
      .map((camera, index) => {
        ...
        const modeStatus = camera.demo_mode
          ? "DEMO"
          : camera.address
          ? "ONLINE"
          : "OFFLINE";
        const modeClass = camera.demo_mode
          ? "status-demo"
          : camera.address
          ? "status-online"
          : "status-warning";

        const detectorIndicator =
          detectorType === "analog" ? "⚙️ ANALOG" : "🔢 DIGITAL";

        console.log(
          `Rendering camera ${camera.id} with detector type: ${detectorType}`
        );
                    ...
                    <div class="feed-info">
                        ...
                        <div class="info-item">
                            ...
                            <span class="reading-value">${
                              camera.demo_mode ? "DEMO" : "DETECTOR"
                            }</span>
                        </div>
                        <div class="info-item">
                            <span>Detector:</span>
...
async function removeCamera(index) {
  if (
    confirm(
      "Are you sure you want to remove this camera? This will also remove it from all team member assignments."
    )
  ) {
    const removedCamera = cameras[index];

    try {
      await apiCall("/api/remove-camera", "POST", {
        camera_id: removedCamera.id,
      });

      addActivityLog(
        `Camera ${removedCamera.id} removed from ${removedCamera.location}`,
        "warning"
      );
      ...
      teamMembers.forEach((member) => {
        if (member.assignedGauges) {
          member.assignedGauges = member.assignedGauges.filter(
            (gaugeId) => gaugeId !== removedCamera.id
          );
        }
      });
...
function updateCameraDisplay(cameraId, newReading) {
  ...
  if (readingElement) {
    const formattedReading =
      typeof newReading === "number" ? newReading.toFixed(1) : newReading;
...
function showAnalogCalibrationModal(cameraId) {
  ...
  calibrationPointsRequired =
    parseInt(document.getElementById("calibrationPointsCount").value) || 4;
  ...
  if (videoFeed) {
    ...
    videoFeed.onload = function () {
      document.getElementById("videoStatus").textContent =
        "Video feed active - Click on gauge numbers";
    };
...
function handleAnalogClick(event) {
  ...
  if (analogCalibrationPoints.length >= calibrationPointsRequired) {
    alert(
      `Maximum calibration points (${calibrationPointsRequired}) already reached. Remove some points or increase the limit.`
    );
    return;
  }
  ...
  console.log(
    `Analog click at: ${actualX}, ${actualY} (frame: ${frameWidth}x${frameHeight})`
  );
...
function undoLastPoint() {
  ...
  if (statusText) {
    const remaining =
      calibrationPointsRequired - analogCalibrationPoints.length;
...
function updateCalibrationPoints() {
  ...
  if (analogCalibrationPoints.length === 0) {
    pointsContainer.innerHTML =
      '<div class="log-placeholder">No calibration points yet - click on gauge numbers to start</div>';
    return;
  }

  const pointsHTML = analogCalibrationPoints
    .map((point, index) => {
...
function updateCalibrationStatus() {
  ...
  if (elements.estimatedRange && collected >= 2) {
    ...
    elements.estimatedRange.textContent = `${min.toFixed(1)} - ${max.toFixed(
      1
    )}`;
  ...
  if (elements.statusText) {
    if (collected === 0) {
      elements.statusText.textContent =
        "Ready for calibration - click on gauge numbers";
      elements.statusText.style.color = "#0369a1";
    } else if (collected < 2) {
      elements.statusText.textContent = `Need ${
        2 - collected
      } more point(s) for minimum calibration`;
      ...
      elements.statusText.textContent =
        "Sufficient points collected - ready to complete!";
...
function completeAnalogCalibration() {
  ...
  console.log(
    "Completing calibration with",
    analogCalibrationPoints.length,
    "points"
  );
    ...
    if (currentAnalogCamera) {
      try {
        const status = await apiCall(
          `/api/analog-status/${currentAnalogCamera}`
        );
...
function updateAnalogStatusDisplay(status) {
  ...
  if (
    elements.statusText &&
    status.status &&
    analogCalibrationPoints.length === 0
  ) {
  ...
  if (elements.pointsCollected && status.calibration_points !== undefined) {
    if (status.calibration_points !== analogCalibrationPoints.length) {
      console.log(
        `Server reports ${status.calibration_points} points, client has ${analogCalibrationPoints.length}`
      );
    }
  }
}
...
function showConfigModal(cameraIndex) {
  ...
  document.getElementById("configDetectorType").value =
    camera.detector_type || "digital";
...
function showValueInputModal() {
  ...
  document.getElementById(
    "clickPositionText"
  ).textContent = `Enter the value shown at position (${pendingCalibrationClick.x.toFixed(
    1
  )}, ${pendingCalibrationClick.y.toFixed(1)}):`;

  document.getElementById("currentPointNumber").textContent = pointNumber;
  document.getElementById(
    "totalPointsNeededText"
  ).textContent = ` of ${calibrationPointsRequired}`;
...
async function submitGaugeValue() {
  ...
  const isDuplicate = existingValues.some(
    (existing) => Math.abs(existing - roundedValue) < tolerance
  );

  if (isDuplicate) {
    if (
      !confirm(
        `Value ${roundedValue} is very close to an existing calibration point. Add anyway?`
      )
    ) {
      return;
    }
      ...
      if (analogCalibrationPoints.length >= calibrationPointsRequired) {
        setTimeout(() => {
          if (
            response.completed ||
            confirm(
              `You've collected ${analogCalibrationPoints.length} of ${calibrationPointsRequired} points. Complete calibration now?`
            )
          ) {
        ...
        setTimeout(() => {
          alert(
            "Calibration completed successfully! The gauge is now ready for use."
          );
          hideAnalogCalibrationModal();
          renderCameras();
        }, 500);
...
function createChartsGrid() {
  ...
  activeCameras.forEach((camera) => {
    ...
    const modeIndicator = camera.demo_mode
      ? "🎭 DEMO"
      : camera.address
      ? "🔴 LIVE"
      : "OFFLINE";
    const statusClass = camera.demo_mode
      ? "status-demo"
      : camera.address
      ? "status-online"
      : "status-warning";
...
function updateIndividualChart(cameraId) {
  ...
  // Draw threshold line
  const threshold =
    camera.threshold || gaugeConfigs[camera.type]?.normalMax || 0;
  const thresholdY =
    padding.top +
    chartHeight -
    ((threshold - minY) / (maxY - minY)) * chartHeight;
  ...
  if (data.length >= 1) {
    ...
    const points = data.map((point, pointIndex) => {
      ...
      const y =
        padding.top +
        chartHeight -
        ((point.value - minY) / (maxY - minY)) * chartHeight;
  ...
  if (currentValue !== undefined && currentValue !== null) {
    ...
    const chartStatusElement = document.getElementById(
      `chart-status-${cameraId}`
    );

    if (chartCurrentElement) {
      chartCurrentElement.textContent = `${
        currentValue.toFixed ? currentValue.toFixed(1) : currentValue
      }${gaugeConfigs[camera.type].unit}`;
    }
...
function updateGaugeReadings() {
  cameras.forEach((camera) => {
    if (camera.demo_mode) {
      ...
      const cutoffTime = now - 30000;
      chartData[camera.id] = chartData[camera.id].filter(
        (point) => point.timestamp > cutoffTime
      );
      ...
      if (isAlerting && !wasAlerting) {
        const assignedMembers = teamMembers
          .filter(
            (member) =>
              member.assignedGauges && member.assignedGauges.includes(camera.id)
          )
    ...
    if (camera.address || camera.demo_mode) {
      ...
      if (reading > threshold * 1.1) {
        if (camera.demo_mode && Math.random() < 0.002) {
          ...
          addActivityLog(
            `Camera ${camera.id} went OFFLINE at ${camera.location}`,
            "warning"
          );
          renderCameras();
          createChartsGrid();
...
function getDetailedStatus(value, camera) {
  ...
  if (value >= dangerHigh)
    return { status: "danger", color: "#ef4444", level: "high" };
  if (value >= warningHigh)
    return { status: "warning", color: "#f59e0b", level: "high" };
  if (value <= dangerLow)
    return { status: "danger", color: "#ef4444", level: "low" };
  if (value <= warningLow)
    return { status: "warning", color: "#f59e0b", level: "low" };
...
function removeTeamMember(index) {
  if (confirm("Are you sure you want to remove this team member?")) {
    ...
    addActivityLog(
      `Team member ${removedMember.name} removed from system`,
      "warning"
    );
...
function updateGaugeCheckboxes() {
  ...
  if (cameras.length === 0) {
    container.innerHTML =
      '<div style="text-align: center; color: #64748b; padding: 1rem;">No cameras available. Add cameras first.</div>';
    return;
  }
