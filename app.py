...
def process_frame_for_detection(frame, camera_id):
    ...
    if gauge_detector and frame is not None:
        try:
            ...
            # Process every 5th frame for CAM01, every 7th for CAM02 to stagger processing
            if camera_id == 'CAM01' and frame_count % 5 == 0:
                threading.Thread(
                    target=gauge_detector.detect_gauge_reading, 
                    args=(frame, camera_id),
                    daemon=True
                ).start()
            elif camera_id == 'CAM02' and frame_count % 7 == 0:
                threading.Thread(
                    target=gauge_detector.detect_gauge_reading, 
                    args=(frame, camera_id),
                    daemon=True
