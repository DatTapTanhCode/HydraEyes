...
class GaugeDetector:
    ...
    def draw_detections(self, frame, detections, number):
        with open('debug_log.txt', 'a') as f:
            ...
            f.write(f"[{timestamp}] draw_detections called - detections: {len(detections) if detections else 0}, number: {number}\n")
    ...
    def get_detection_overlay(self, frame, camera_id):
        ...
        if frame is None:
            with open('debug_log.txt', 'a') as f:
                f.write(f"[{timestamp}] Frame is None")
        ...
        if camera_id in self.last_detections and frame is not None:
            ...
            with open('debug_log.txt', 'a') as f:
                f.write(f"[{timestamp}] Found detection data: {detection_data['number']}\n")
